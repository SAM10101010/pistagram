...
class ShareReelChatScreen extends StatefulWidget {
  final String reelId;
  const ShareReelChatScreen({super.key, required this.reelId});

  @override
  State<ShareReelChatScreen> createState() => _ShareReelChatScreenState();
}

class _ShareReelChatScreenState extends State<ShareReelChatScreen> {
  ...
  void _filter() {
    ...
    setState(() {
      if (query.isEmpty) {
        ...
        _filtered = _chatUsers.where((cu) => cu.user.username.toLowerCase().contains(query) || cu.user.displayName.toLowerCase().contains(query)).toList();
      }
    });
  }
    ...
    try {
      ...
      for (final chatUser in _chatUsers) {
        if (_selected.contains(chatUser.user.uid)) {
          await _messaging.sendMessage(
            ...
            text: 'Check out this reel! pistagram://reel/${widget.reelId}',
            mediaUrl: '',
          );
        }
      }
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Sent to ${_selected.length} ${_selected.length == 1 ? 'person' : 'people'}!'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        elevation: 0,
        title: Text('Share Reel', style: GoogleFonts.outfit(fontWeight: FontWeight.bold, color: textColor)),
          ...
          if (_selected.isNotEmpty)
            TextButton(
              ...
              child: _sending
                  ...
                  : Text('Send (${_selected.length})', style: GoogleFonts.inter(fontWeight: FontWeight.w600, color: accent)),
            ),
        ],
      ),
      body: _loading
                          ...
                          itemBuilder: (ctx, i) {
                            ...
                            return ListTile(
                              ...
                              trailing: Icon(
                                ...
                                color: isSelected ? accent : subColor.withAlpha(80),
                              ),
                            );
                          },
                        ),
