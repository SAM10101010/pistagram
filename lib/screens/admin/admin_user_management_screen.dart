      ...
      builder: (ctx) => SafeArea(
        child: Column(
          ...
          children: [
            ...
            Text(user.username, style: GoogleFonts.outfit(fontSize: 18, fontWeight: FontWeight.bold, color: textColor)),
            Text('Status: ${user.accountStatus}', style: GoogleFonts.inter(fontSize: 13, color: Colors.grey)),
            const SizedBox(height: 12),
            ListTile(
              leading: const Icon(Icons.person),
              title: Text('View Profile', style: GoogleFonts.inter(color: textColor)),
              onTap: () {
                Navigator.pop(ctx);
            ...
            if (user.accountStatus == 'active')
              ListTile(
                ...
                title: Text('Suspend', style: GoogleFonts.inter(color: Colors.orange)),
                onTap: () async {
                  Navigator.pop(ctx);
              ...
              ListTile(
                ...
                title: Text('Unsuspend', style: GoogleFonts.inter(color: Colors.green)),
                onTap: () async {
                  Navigator.pop(ctx);
            ...
            ListTile(
              ...
              title: Text('Temporary Ban (7 days)', style: GoogleFonts.inter(color: Colors.orange)),
              onTap: () async {
                Navigator.pop(ctx);
                await _moderation.temporaryBan(user.uid, const Duration(days: 7));
                _load();
              },
            ),
            ListTile(
              leading: const Icon(Icons.block, color: Colors.redAccent),
              title: Text('Permanent Ban', style: GoogleFonts.inter(color: Colors.redAccent)),
              onTap: () async {
                final confirmed = await showDialog<bool>(
                  context: context,
                  builder: (dCtx) => AlertDialog(
                    title: const Text('Permanent Ban'),
                    content: Text('Permanently ban ${user.username}? This cannot be undone.'),
                    actions: [
                      TextButton(onPressed: () => Navigator.pop(dCtx, false), child: const Text('Cancel')),
                      TextButton(onPressed: () => Navigator.pop(dCtx, true), child: const Text('Ban', style: TextStyle(color: Colors.redAccent))),
                    ],
                  ),
                );
                if (confirmed == true) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        elevation: 0,
        title: Text('User Management', style: GoogleFonts.outfit(fontWeight: FontWeight.bold, color: textColor)),
            ...
            child: _loading
                ...
                : _users.isEmpty
                    ...
                    : ListView.builder(
                        ...
                        itemBuilder: (ctx, i) {
                          final user = _users[i];
                          final statusColor = user.accountStatus == 'active' ? Colors.green : Colors.redAccent;
                          return ListTile(
                            ...
                            subtitle: Text(user.email, style: GoogleFonts.inter(fontSize: 12, color: subColor)),
                            trailing: Container(
