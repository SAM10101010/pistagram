...
exports.checkSecretReelLimit = void 0;
const functions = __importStar(require("firebase-functions"));
const admin = __importStar(require("firebase-admin"));
if (!admin.apps.length)
    admin.initializeApp();
exports.checkSecretReelLimit = functions.firestore
    .document('reels/{reelId}')
    .onUpdate(async (change) => {
    const after = change.after.data();
    const before = change.before.data();
    // Only check if views changed and reel is limited
    if (!after.isLimited || !after.isActive)
        return;
    if (after.viewsCount === before.viewsCount)
        return;
...
//# sourceMappingURL=checkSecretReelLimit.js.map